...
//
static
ULONG NumberOfSleepThreads;


//
// Counter for number of thread switch
//
static
ULONG SwitchCount;
...
VOID Schedule () {
	PUTHREAD NextThread;
	PUTHREAD AwakenThread = NULL ;

	if (NumberOfSleepThreads > 0)
		AwakenThread = (PUTHREAD) UtAwake();

	if ((AwakenThread==NULL) && (IsListEmpty(&ReadyQueue))) {
		while (AwakenThread == NULL)
			AwakenThread = (PUTHREAD) UtAwake();
		NextThread = AwakenThread ;
	}
	else if ((AwakenThread==NULL) && (!IsListEmpty(&ReadyQueue)))
		NextThread = ExtractNextReadyThread();
	else NextThread = AwakenThread ;
...
VOID UtInit() {
	...
	//InitializeListHead((PLIST_ENTRY)&SleepQueue);
}
...
//
// Puts the current user thread to sleep for the specified time, and 
// another thread is called.
//
DWORD UtSleep(DWORD sleepTimeInMillis) {
	DWORD timeStart= GetTickCount(), timeEnd, timeElapsed ;

	PSLEEP_NODE newSleepNode = (PSLEEP_NODE) malloc(sizeof(SLEEP_NODE)) ;
	InitializeSleepNode(newSleepNode,timeStart+sleepTimeInMillis);

	if (NumberOfSleepThreads==0)
		SleepQueue=newSleepNode;
	else
		SleepQueue->NextSleepNode=newSleepNode;
	NumberOfSleepThreads++;
	//Needs to change to another thread
	Schedule() ;
...
//
// Returns the number of thread swiches
//
...
//
// Returns true specified thread is active
//
...
//
// Awakes the first thread that has slept the agreed time.
//
HANDLE UtAwake() {
	PSLEEP_NODE SleepQueueIterator = SleepQueue ;
	PSLEEP_NODE PreviousSleepNode = NULL ;

	if (NumberOfSleepThreads == 0)
		return NULL ;

	do	{
		if (SleepQueueIterator->expectedTimeToEnd < GetTickCount()) 
		{
			if (PreviousSleepNode!=NULL) {
				PreviousSleepNode->NextSleepNode = SleepQueueIterator->NextSleepNode;
			}
			else {
				SleepQueue= SleepQueueIterator->NextSleepNode;
			}
			NumberOfSleepThreads--;
			return (PUTHREAD) SleepQueueIterator->Thread ;
		}
		else 
		{
			PreviousSleepNode = SleepQueueIterator;
			SleepQueueIterator=SleepQueueIterator->NextSleepNode;
		}
	} while (SleepQueueIterator != NULL);
