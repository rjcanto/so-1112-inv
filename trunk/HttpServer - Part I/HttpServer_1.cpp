...
UINT WINAPI ProcessConnection(LPVOID arg) {
	...
	while(GetQueuedCompletionStatus(hCompletionPort, &nBytes, (PULONG_PTR)&connectSocket, &ov, INFINITE)) {
...
int _tmain (int argc, LPCTSTR argv [])
{
	...
	hCompletionPort = CreateIoCompletionPort(INVALID_HANDLE_VALUE, NULL, 0, MIN_WORKINGTHREADS);
	if(!hCompletionPort) {
