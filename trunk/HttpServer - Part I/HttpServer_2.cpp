...
UINT WINAPI ProcessConnection(LPVOID arg) {
	HANDLE			hCompletionPort = (HANDLE) arg;
	...
	do {
		if(!GetQueuedCompletionStatus(hCompletionPort, &nBytes, (PULONG_PTR)&connectSocket, &ov, 15000)) {
			if(GetLastError() == TIMEOUT_ERROR) {
				EnterCriticalSection(&cs);
				if(threadsCounter > MIN_WORKINGTHREADS) {
					// Thread vai terminar decrementar
					threadsCounter--;
				...
				// Outro tipo de erro
				threadsCounter--;
				_tprintf(_T("Error: GetQueuedCompletionStatus!\n"));
				break;
			}
		}
...
int _tmain (int argc, LPCTSTR argv [])
{
	...
	/* Criar o I/O Completion Port. */
	/* Alteração para MAX_WORKINGTHREADS para a segunda parte da Fase 1 */
	hCompletionPort = CreateIoCompletionPort(INVALID_HANDLE_VALUE, NULL, 0, MAX_WORKINGTHREADS);
	if(!hCompletionPort) {
	...
	/* Criar o número mínimo de worker threads */
	threadsCounter = MIN_WORKINGTHREADS;
	for (int i = 0; i < MIN_WORKINGTHREADS; ++i)
		_beginthreadex(NULL, 0, ProcessConnection, hCompletionPort, 0, NULL);

	/* Main thread becomes listening/connecting/monitoring thread */
	/* Find an empty slot in the server thread arg array */
	_tprintf(_T("Version I/O Completion Port - Part 1.\n"));
	while (!terminate) {
		...
		EnterCriticalSection(&cs);
		if(threadsCounter < MAX_WORKINGTHREADS && threadsCounter == workersCounter) {
			++threadsCounter;
			_beginthreadex(NULL, 0, ProcessConnection, hCompletionPort, 0, NULL);
			LeaveCriticalSection(&cs);
		}
		...
		PostQueuedCompletionStatus(hCompletionPort, 0, connectSock, &ov);
	}
